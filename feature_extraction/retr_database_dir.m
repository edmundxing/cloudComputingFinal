function [database] = retr_database_dir(rt_data_dir, ext)
%=========================================================================
% inputs
...
for ii = 1:length(subfolders),
    ...
    if ~strcmp(subname, '.') && ~strcmp(subname, '..'),
        ...
        frames = dir(fullfile(rt_data_dir, subname, ['*.' ext]));
        c_num = length(frames);
        database.subnumber(database.nclass) = c_num;            
        ...
        for jj = 1:c_num,
            ...
            database.imgname = [database.imgname, frames(jj).name];
            database.path = [database.path, c_path];
            database.subpath = [database.subpath, ['\' subname, '\' frames(jj).name ]];
        end;    
    end;
end;
disp('done!');